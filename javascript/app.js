$(document).ready(function() {
    if (sessionStorage.getItem("email") !== null) {
        ...
        */
    var User = function(fname, lname, email, p1, p2, mob) {
            this.fname = fname;
            this.lname = lname;
            this.email = email;
            this.p1 = p1;
    ...
    */
    $(document).on("submit", "#signup-form", (function(event) {
        ...
        if (validateForm()) {
            ...
            try {
                userarray = JSON.parse(getCookie("detail")) || [];
                console.log("tryyyyyyyyyy", userarray);
            } catch (e) {
                userarray = [];
                console.log("catch", userarray);
            }
            var userobj = new User(firstName, lastName, emailAddress, creatPassword, confirmPassword, newMobNo);
            userarray.push(userobj);
            console.log(userobj);
            var userobjson = JSON.stringify(userarray);
            console.log(userobjson);
             result = setCookie("detail", userobjson, 1);
            if (result) {
                $("#result").text("your data is saved");
            } else {
                $("#result").text("your data is not saved");
            }
        }
        event.preventDefault();
    }));
